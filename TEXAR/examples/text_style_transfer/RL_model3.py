# convnet+  final_state from embedding
...
#RL_model3.py 和 RL_model.py 的不同是： 3里面在female 和 male的 dense layer之前concact了UniDirectionalRNN encoder的final state
        
class RLModel3(object):
    ...
    def _build_model(self, inputs, vocab, finputs, minputs,gamma):
        ...
        self.RL_fenc_outputs, self.RL_ffinal_state = self.fencoder(
            self.freg_embedder(self.outputs.sample_id),sequence_length=tf.map_fn(lambda x:tf.size(x),self.outputs.sample_id))
        self.RL_freg_output = self.fdense_layer(inputs = tf.concat([self.RL_fconv_output, self.RL_ffinal_state], -1))
        ...
        self.RL_menc_outputs, self.RL_mfinal_state = self.mencoder(
            self.mreg_embedder(self.outputs.sample_id),sequence_length=tf.map_fn(lambda x:tf.size(x),self.outputs.sample_id))
        self.RL_mreg_output = self.mdense_layer(inputs = tf.concat([self.RL_mconv_output, self.RL_mfinal_state], -1))
        ...
        self.YRL_fenc_outputs, self.YRL_ffinal_state = self.fencoder(
            self.freg_embedder(self.outputs.sample_id),sequence_length=tf.map_fn(lambda x:tf.size(x),self.outputs.sample_id))
        self.YRL_freg_output = self.fdense_layer(inputs = tf.concat([self.YRL_fconv_output, self.YRL_ffinal_state], -1))
        ...
        self.YRL_menc_outputs, self.YRL_mfinal_state = self.mencoder(
            self.mreg_embedder(self.outputs.sample_id),sequence_length=tf.map_fn(lambda x:tf.size(x),self.outputs.sample_id))
        self.YRL_mreg_output = self.mdense_layer(inputs = tf.concat([self.YRL_mconv_output, self.YRL_mfinal_state], -1))
