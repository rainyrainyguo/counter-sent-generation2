...
class RNN_VAE(nn.Module):
    ...
    def forward(self,t):
        ...
        sample_idxs, ally = model.sample_sentence(z,temp=t)
...
def save_model():
    ...
    torch.save(model.state_dict(), 'models/{}.bin'.format('pre_yelp128_100_addsamplepredif'))   
...
for it in range(600000):
    ...
    #z = model.sample_z_prior(1)
    #sample_idxs, ally = model.sample_sentence(z)
    #sample_sent = ' '.join([TEXT.vocab.itos[i] for i in sample_idxs])
    #sample_idxs = torch.tensor(sample_idxs,device=inputs.device)
    new_recon_loss,sample_sent = model.forward(t)
    if t>1:
        t=t-0.00001
    ...
    if sample_sent:
        ...
        #record.append([pre_dif_sample,sample_sent])
    else:
        pre_dif_sample = 1/pre_weight
    
    #loss = (recon_loss + kld_weight * kl_loss)*(pre_weight*pre_dif_sample)
    loss = new_recon_loss*(pre_weight*pre_dif_sample)
    ...
    if it%50==0:
        ...
        '''
        print('Iter-{}; Loss: {:.4f}; new_Recon: {:.4f}; Grad_norm: {:.4f};'
              .format(it, loss.data[0], new_recon_loss.data[0], grad_norm))
        
        print('Sample: "{}"'.format(sample_sent))
        if sample_sent:
            print("mrnn sample prediction: ",m0)
            print("frnn sample prediction: ",f0)
    ...
    if it%1000==0:
        print("saving model pre_yelp128_100_addsamplepredif.bin")
        print("\n")
        save_model()
        
'''        
with open('addedsamplepredif_predif_sample','w') as f:
    for x in record:
        f.write(str(x))
        f.write('\n')
'''