...
###################################################################
        pre_dif=torch.empty(mbsize)
        for i in range(mbsize):
            sent = ' '.join([TEXT.vocab.itos[i] for i in model.sample_sentence(z[0].unsqueeze(0))])
            if sent:
                mscore = predict_sentiment(sent,mrnn,mTEXT)
                fscore = predict_sentiment(sent,frnn,fTEXT)
                dif = mscore-fscore
                pre_dif[i] = dif*dif
      
        pre_dif = torch.mean(pre_dif)
        #print("mean square dif: ",pre_dif.item())
###################################################################        
        ...
        return recon_loss, kl_loss,pre_dif
...
print("loading dataset female_sent_obftrain_less200.tsv...")
train  = data.TabularDataset.splits(
        path='../sent/ori_gender_data/', 
        train='female_sent_obftrain_less200.tsv',
        format='tsv',
...
model.load_state_dict(torch.load('models/{}.bin'.format('200yelp_nofix_max40_predif')))
    
def save_model():
    if not os.path.exists('models/'):
        os.makedirs('models/')

    torch.save(model.state_dict(), 'models/{}.bin'.format('200yelp_nofix_max40_predif'))   
...
for it in range(200000):
    ...
    recon_loss, kl_loss,pre_dif = model.forward(inputs)
    pre_dif = pre_dif.cuda()
    ...
    if it%100==0:
        ...
        print("mrnn prediction: ",m)
        print("frnn prediction: ",f)
        print("abs dif: ",abs(m-f))
        print("square dif: ",(m-f)*(m-f))
        print("\n")
    ...
    if it%2000==0:
        print("saving model 200yelp_nofix_max40_predif.bin")
        print("\n")
        save_model()
