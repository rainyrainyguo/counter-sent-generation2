...
print("loading dataset female_sent_tmp_train.tsv")
train  = data.TabularDataset.splits(
        path='../sent/ori_gender_data/',
        train='female_sent_tmp_train.tsv',
        format='tsv',
...
f = open('female_sent_tmp_train_vocab_fcnn3.txt','w')
...
try:
    for epoch in range(N_EPOCHS):
        ...
        print("saving model...fcnn3")
        torch.save(model,'fcnn3')
...
fpref = [predict_sentiment(x) for x in fs]
fprem = [predict_sentiment(x) for x in ms]

print("writing fpref to file...")
with open('fpref_fcnn3.txt', 'w') as f:
    f.write(fpref)
print("writing fprem to file...")
with open('fprem_fcnn3.txt', 'w') as f:
    f.write(fprem)
