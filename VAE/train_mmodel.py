...
print("loading dataset random1_sent_obftrain_less700.tsv...")
train  = data.TabularDataset.splits(
        path='../sent/ori_gender_data/', 
        train='random1_sent_obftrain_less700.tsv',
        format='tsv',
        fields=[('Text', TEXT),('Label', LABEL)])[0]

TEXT.build_vocab(train, max_size=60000, vectors="fasttext.en.300d")
...
import json
with open('sent_tmp_train_vocab_random1','w') as f:
...
train, valid = train.split(split_ratio=0.999)
...
try:
    for epoch in range(N_EPOCHS):
        ...
        print("saving model:   random1")
        torch.save(model,'random1')
...
mprem = [predict_sentiment(x,model) for x in ms]
mpref = [predict_sentiment(x,model) for x in fs]
...
print("writing rpref to file random1_pref.txt...")
with open('random1_pref.txt','w') as f:
    f.write(str(mpref))
print("writing rprem to file random1_prem.txt...")
with open('random1_prem.txt','w') as f:
    f.write(str(mprem))
